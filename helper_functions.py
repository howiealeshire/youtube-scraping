...
def is_in_dictlist_column(dict_list, column_name, value) -> bool:
    """@safe"""
    db_values = [d[column_name] for d in dict_list]
    if value in db_values: return True
    return False



def get_unexported_vals_from_list(vals: List[str], column_name: str, db_vals: List[Dict]) -> List[str]:
     unexported_vals = []
     for elem in vals:
...
def get_dict_list_column(dict_list,column_name):
    return [d[column_name] for d in dict_list]

def load_db_table(conn,table_name):
    channels = Table(table_name)
...
def addResponseToDB(connection, response, pg_table=''):
    ...
    if pg_table != '':
        if kind is not None:
            response.pop('kind')
        addDictToDB(connection, response, pg_table)
...
def addDictToDB(connection, response, pg_table):
...
def write_dictlist_to_db(conn, dict_list, pg_table):
    for this_dict in dict_list:
        addDictToDB(conn, this_dict, pg_table)
...
def is_basename(filepath):
    """@safe"""
    if '\\' in filepath or '/' in filepath: return False
    else: return True
