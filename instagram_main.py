...
def build_follower_count_dict():
    ...
    for elem in onlyfiles:
        read_path = join('/add_already_existing_follower_counts3', elem)
    ...
    for d in csv_dict_list_list:
        finalMap.update(d)
    json.dump(finalMap, open('follower_counts3.json','w'))
    pprint(finalMap)
    return finalMap

def write_follower_counts_to_files():
    mypath = 'C:/Users/howie/PycharmProjects/pythonProject/add_already_existing_follower_counts2'
    onlyfiles = [f for f in listdir(mypath) if isfile(join(mypath, f))]
    user_id_follower_count_dict = json.load(open('C:/Users/howie/PycharmProjects/pythonProject/follower_counts2.json','r'))
    for elem in onlyfiles:
